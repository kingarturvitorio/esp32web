...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-a1-on').addEventListener('click', () => {
    sendWS({ comando: 'atuador1', valor: 'on' });
  });
  document.getElementById('btn-a1-off').addEventListener('click', () => {
    sendWS({ comando: 'atuador1', valor: 'off' });
  });
  document.getElementById('btn-a1-ang').addEventListener('click', () => {
    const ang = document.getElementById('input-a1-ang').value;
    sendWS({ comando: 'atuador1', valor: ang });
  });
  function sendA2(v) {
    const ident = document.getElementById('input-ident').value.trim();
    const payload = ident ? { comando: 'atuador2', valor: v, identificador: ident }
      : { comando: 'atuador2', valor: v };
    sendWS(payload);
  }
  document.getElementById('btn-a2-on').addEventListener('click', () => sendA2('on'));
  document.getElementById('btn-a2-off').addEventListener('click', () => sendA2('off'));
